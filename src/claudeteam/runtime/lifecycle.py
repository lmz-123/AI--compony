...
def _seed_codex_config(agent: str) -> None:
    """Copy the operator's Codex provider config into a fresh agent home.

    Codex agents use an isolated ``CODEX_HOME``.  Seeding only ``auth.json``
    leaves a fresh agent unaware of custom providers such as an OpenAI-
    compatible gateway, so it silently falls back to the default OpenAI
    endpoint.  Copy the config once, before ``ensure_workdir_trusted`` appends
    the project trust entry.  Existing per-agent config is never overwritten.
    """
    from claudeteam.runtime.paths import agent_home as _agent_home
    src = Path.home() / ".codex" / "config.toml"
    dst = Path(_agent_home(agent)) / ".codex" / "config.toml"
