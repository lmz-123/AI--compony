...
def main(argv: list[str]) -> int:
    ...
    reap_agents = bool(tunables.tunable("watchdog.reap_dead_agents", True))
    reap_cooldown_s = float(tunables.tunable("watchdog.agent_respawn_cooldown_s", 300))
    print(f"🐕 watchdog supervising {[s.name for s in specs]} every "
          f"{check_interval_s}s ({alert_msg}; agent-reap "
          f"{'on' if reap_agents else 'off'})")
