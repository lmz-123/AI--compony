...
def init_prompt(agent: str) -> str:
    ...
    base = (
        ...
        f"  2.5 If an unread item is clearly stale history from a previous round (old ready/report-in/completion chatter, duplicate summary,\n"
        f"      or already-settled coordination) do not repost it to chat on init/reidentify. Clean it up quietly with `claudeteam read <local_id>`.\n"
    ...
    if agent == "manager":
        ...
        base += (
            ...
            "  • On init/reidentify, never rebroadcast historical ready/report-in/roll-call/summary messages just because they are still unread.\n"
            "    Only post if the message is still actionable for the current round.\n"
