...
本仓库内置三角色服务器编制：主管负责需求分析、拆解和验收，开发员工负责编码与测试，部署员工通过受控 SSH 清单执行拉取、部署、健康检查和回滚。服务器宿主机不需要预装 Codex，Docker 镜像已经包含 Codex CLI，通过 Git 忽略的 API Key 访问远程 Responses 兼容中转站。

- 团队模板：[`templates/ai-company/`](../templates/ai-company/)
- 服务器部署说明：[`deploy/server/README_zh.md`](../deploy/server/README_zh.md)
- 服务器 Compose：[`deploy/server/compose.yaml`](../deploy/server/compose.yaml)
