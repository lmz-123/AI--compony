...
def test_ensure_agent_home_seeds_codex_provider_config_once():
    """A fresh CODEX_HOME inherits the operator's custom provider config.

    Without this copy, API-key deployments in Docker have credentials but
    Codex still calls the default OpenAI endpoint.
    """
    from claudeteam.agents.codex_cli import codex_home
    team = {"agents": {"developer": {"cli": "codex-cli"}}}
    with isolated_env(team=team):
        operator = Path(paths.state_dir()).parent / "operator"
        ...
        with env_patch(HOME=str(operator)):
            lifecycle._ensure_agent_home("developer", "codex-cli")
            destination = Path(codex_home("developer")) / "config.toml"
            assert destination.read_text() == source.read_text()

            destination.write_text('model_provider = "agent-specific"\n')
            lifecycle._ensure_agent_home("developer", "codex-cli")
            assert destination.read_text() == 'model_provider = "agent-specific"\n'
